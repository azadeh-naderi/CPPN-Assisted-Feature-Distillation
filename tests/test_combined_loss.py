...
def test_cppn_weight_gives_full_alpha_to_soft_and_independent_weight_to_cppn():
    loss_hard = torch.tensor(2.0)
    loss_soft = torch.tensor(1.0)
    cppn_losses = [torch.tensor(3.0), torch.tensor(5.0)]
    result = combined_loss(loss_hard, loss_soft, cppn_losses, alpha=0.9, cppn_weight=0.3)
    expected = 0.1 * loss_hard + 0.9 * loss_soft + 0.3 * 4.0  # mean(3.0, 5.0) == 4.0
    assert torch.allclose(result, expected)
...
def test_cppn_weight_ignored_when_no_cppn_losses():
    loss_hard = torch.tensor(2.0)
    loss_soft = torch.tensor(1.0)
    result = combined_loss(loss_hard, loss_soft, [], alpha=0.9, cppn_weight=0.3)
    assert torch.allclose(result, 0.1 * loss_hard + 0.9 * loss_soft)
